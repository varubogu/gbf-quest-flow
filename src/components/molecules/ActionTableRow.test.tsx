...
describe('ActionTableRow', () => {
  ...
  beforeEach(() => {
    rendered = render(<ActionTableRow {...defaultProps} />);
  });

  it('通常モードで正しく表示される', () => {
    // 各セルの内容が表示されていることを確認
    const hpCell = screen.getByTestId('cell-hp');
    const chargeCell = screen.getByTestId('cell-charge');
  ...
  it('編集モードで追加・削除ボタンが表示される', () => {
    rendered.rerender(<ActionTableRow {...defaultProps} isEditMode={true} />);
  ...
  it('現在の行のスタイルが正しく適用される', () => {
    ...
    // 現在の行
    rendered.rerender(<ActionTableRow {...defaultProps} isCurrentRow={true} />);
